...
void zns_raid_set_block_size(int block_size);
void zns_raid_set_system_mode(int mode);
void zns_raid_set_raid_parameters(int n_data_blocks, int n_parity_blocks, uint32_t* unitSizes, int raid_level);
void zns_raid_set_gc(bool enable);
void zns_raid_set_sync_group_sizes(uint32_t* group_sizes);
void zns_raid_set_stripe_unit_sizes(uint32_t* block_sizes);
