...
void RequestContext::UpdateContinousPba() {
  if (pbaArray.size() < targetBytes / Configuration::GetBlockSize()) {
    // used for GC reads
    pbaArray.resize(targetBytes / Configuration::GetBlockSize());
  }
  for (int i = 0; i < targetBytes; i += Configuration::GetBlockSize()) {
    PhysicalAddr addr;
    addr.segment = segment;
    addr.zoneId = zoneId;
    addr.offset = offset + i / Configuration::GetBlockSize();
    pbaArray[i / Configuration::GetBlockSize()] = addr;
  }
}
...
uint64_t simpleHash(uint8_t* data) {
  uint64_t hash = 0;
  for (int i = 0; i < Configuration::GetBlockSize() / 8; i++) {
...
void RequestContext::addMsg(std::string s) { 
  ...
  lockInt = 1;
  std::string input = s + " " + std::to_string(threadId);
  if (mMsgSet.find(input) != mMsgSet.end()) {
    lockInt = 0;
    return;
  }
  mMsgSet.insert(input);
  mMsg.push(input);
  if (mMsg.size() > mMsgSize) { 
    mMsgSet.erase(mMsg.front());
    mMsg.pop();
  }
  lockInt = 0;
}

void RequestContext::printMsg() {
  printf("print message:\n");
  while (mMsg.empty() == false) {
    printf("%s\n", mMsg.front().c_str());
    mMsgSet.erase(mMsg.front());
    mMsg.pop();
  }
...
uint32_t RequestContext::numMsg() {
  return mMsg.size();
}
...
RequestContextPool::RequestContextPool(uint32_t cap) {
  ...
  for (uint32_t i = 0; i < capacity; ++i) {
    ...
    contexts[i].dataBuffer = (uint8_t *)spdk_zmalloc(
        Configuration::GetMaxStripeUnitSize(), 4096,
        NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
    contexts[i].metadataBuffer = (uint8_t *)spdk_zmalloc(
        Configuration::GetMaxStripeUnitSize() / Configuration::GetBlockSize() * 
        Configuration::GetMetadataSize(),
        Configuration::GetBlockSize(),
        NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
    contexts[i].bufferSize = Configuration::GetMaxStripeUnitSize();
    ...
    if (!availableContexts.empty()) {
      ...
      ctx->metadataBuffer = (uint8_t *)spdk_zmalloc(
          ...
          * Configuration::GetMetadataSize(),
          Configuration::GetBlockSize(),
          NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
      ctx->bufferSize = Configuration::GetMaxStripeUnitSize(); 
      ctx->Clear();
      ctx->available = false;
...
void DecodeStripe(uint32_t offset, uint32_t stripeId, 
    uint8_t **stripe, bool *alive, uint32_t n, uint32_t k,
    uint32_t decodeZid, uint32_t unitSize)
{
  ...
  for (uint32_t i = 0; i < n; ++i) {
    uint32_t zid = Configuration::CalculateDiskId(stripeId, i, raidLevel, n
      /*offset, i, raidLevel, n*/);
    if (alive[zid]) {
      mapping[i] = zid;
    } else {
      mapping[i] = ~0u;
...
void DumpLatencies(std::map<uint32_t, uint32_t>& latCnt, const char* name) {
  ...
  // append (in memory)
  for (auto it = latCnt.begin(); it != latCnt.end(); ++it) {
    total_cnt += it->second;
  ...
  printf("%s avg: %.2lf\n", name, (double)total_time / total_cnt);
}
