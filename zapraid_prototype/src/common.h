...
struct RequestContext
{
  ...
  uint32_t pbaInBlk; // used by index map, but need to be transferred to pbaArray
  std::vector<PhysicalAddr> backupPbaArray;
  uint32_t nValid = 0; // valid blocks; 0 for normal reads, < read size for partial gc reads
